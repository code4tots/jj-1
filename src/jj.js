...
class CodeGenerator {
  ...
  getContext() {
    return "." + this._contextStack.join(".");
  }
  pushContext(part) {
  ...
  getDebugMessageFromToken(token) {
    return this.getContext() + "@" + token.source.uri + "@" +
           token.getLineNumber();
  }
  ...
  translateStatement(node) {
    ...
    case "ExpressionStatement":
      return "\n" + this.translateExpression(node.expr) + ";";
  ...
  translateInnerExpression(node) {
    ...
    case "Function": {
      ...
      this.pushContext(node.name === null ? ".*" : node.name);
      try {
  ...
  try {
    ...
    if (done) {
      ...
      value.then(result => {
        ...
        asyncfHelper(generatorObject, resolve, reject, reason, thr);
      });
    }
  } catch (e) {
    reject(e);
